...
(function() {
  var MONGO_LOG_SERVER_URL, MongoClient, SERVER_VERSION, accessControl, asyncAdmin, dashboardRenderer, express, fs, http, https, main, mongodb, sessionHandler, startup, url;
  ...
  startup = function(ver) {
    return MongoClient.connect(MONGO_LOG_SERVER_URL, function(err, db) {
      var d;
      if (err) {
        throw err;
      }
      ...
      return db.collection("startup").insertOne(d, function(err, res) {
        if (err) {
          throw err;
        }
        return db.close;
  ...
  main = function() {
    ...
    expressApp.post('/admin/async/user/add', function(req, res) {
      return res.send("OK :)");
    });
    expressApp.get('/admin/async/user/*', asyncAdmin.getUser);
    expressApp.get('/api/version', function(req, res) {
      var ct;
      ct = {
        "Content-Type": "application/json"
      };
      res.writeHead(200, ct);
      return res.end(JSON.stringify(SERVER_VERSION));
    });
